...

{
...

    {
        ...
        if (!string.IsNullOrWhiteSpace(_kafkaOAuth.LogicalCluster))

        {

            cfg["sasl.oauthbearer.extensions"] = $"logicalCluster={_kafkaOAuth.LogicalCluster.Trim()}";
...

    {
...

        {
            ...
            missing.Add(nameof(KafkaOAuthOptions.TokenEndpointUrl));
...

        {
            ...
            missing.Add(nameof(KafkaOAuthOptions.ClientId));
...

        {
            ...
            missing.Add(nameof(KafkaOAuthOptions.ClientSecret));
...

        {
            ...
            $"Invalid Kafka OAuth token endpoint '{_kafkaOAuth.TokenEndpoint}'. Expected an absolute URL.");
...

    {
...

        {
            ...
            KafkaOAuthOptions nested = options.OAuth;

            return new OAuthConfig(

                TokenEndpoint: nested.TokenEndpointUrl,

                ClientId: nested.ClientId,

                ClientSecret: nested.ClientSecret,

                Scope: nested.Scope,

                LogicalCluster: nested.LogicalCluster,

                IdentityPoolId: nested.IdentityPoolId);
