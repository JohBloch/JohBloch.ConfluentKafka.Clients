namespace JohBloch.ConfluentKafka.Clients.Services.Serialization.Avro
{
    ...
    public class AvroDeserializer<T> : IMessageDeserializer<T>
    {
        ...
        public AvroDeserializer(JohBloch.ConfluentKafka.SchemaRegistryExtClient.Interfaces.ISchemaRegistryExtClient schemaRegistry, ILogger<AvroDeserializer<T>> logger)
        {
        ...
        public async Task<T> DeserializeAsync(byte[] data, SerializationContext context)
        {
            try
            {
                ...
                var deserializer = new AsyncSchemaRegistryDeserializer<T>(client);
                return await deserializer.DeserializeAsync(data, false, context).ConfigureAwait(false);
            }
